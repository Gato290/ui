...
function ElementsModule.Initialize(color, saveFunc, config, autoSave)
...
-- ============================================
function ElementsModule.AddDropdown(parent, config, countItem, countDropdown, moreBlur, dropdownSelect, dropdownPage, dropPageLayout, updateSizeCallback)
    config = config or {}
    config.Title = config.Title or "Title"
    config.Content = config.Content or ""
    ...
    -- SAFETY CHECK
    if not moreBlur or not dropdownSelect or not dropdownPage or not dropPageLayout then
        warn("Dropdown Error: Required UI elements are nil")
        return {
            Clear = function() end,
    ...
    -- Buat Dropdown Frame di parent (SectionAdd)
    ...
    DropdownContainer.Name = "DropdownContainer_" .. tostring(countDropdown)
    DropdownContainer.Parent = dropdownPage
    ...
    DropdownButton.Activated:Connect(function()
        if not moreBlur.Visible then
            ...
            dropPageLayout:JumpToIndex(DropdownContainer.LayoutOrder)
            ...
            -- Refresh options when opened
            DropdownFunc:RefreshOptions()
        end
    end)

    -- Close dropdown when clicking outside
    moreBlur.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 and moreBlur.Visible then
    ...
    function DropdownFunc:RefreshOptions()
        ...
        for _, Drop in ScrollSelect:GetChildren() do
            if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") then
                ...
                if config.Multi then
                    if type(DropdownFunc.Value) == "table" then
                        for _, val in ipairs(DropdownFunc.Value) do
                            if val == v then
                                selected = true
                                break
                            end
    ...
    function DropdownFunc:Set(Value)
        ...
        -- Update all options
        for _, Drop in ScrollSelect:GetChildren() do
            if Drop.Name == "Option" and Drop:FindFirstChild("OptionText") and Drop:FindFirstChild("ChooseFrame") then
                local v = Drop:GetAttribute("RealValue")
                local selected = false
