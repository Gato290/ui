...
-- ✅ Save config dengan nama custom
function SaveConfigAs(name)
    if not name or name == "" then return end
    local path = ConfigFolder .. "/Config/" .. name .. ".json"
    ConfigData._version = CURRENT_VERSION
    if writefile then
...
-- ✅ Ambil list semua config yang tersimpan
function GetConfigList()
    local list = {}
    if listfiles then
        local files = listfiles(ConfigFolder .. "/Config")
...
function Chloex:Window(GuiConfig)
    ...
    -- ✅ SetupConfig: buat section config siap pakai
    function GuiFunc:SetupConfig(Section, DropdownElement)
        local configName = game.Players.LocalPlayer.Name
        local selectedConfig = ""

        -- Refresh list config di dropdown
        local function RefreshDropdown()
            if DropdownElement and DropdownElement.Refresh then
                DropdownElement:Refresh(GetConfigList())
            end
        end

        Section:AddInput({
            Title = "Config Name",
            Content = "Enter your config name",
            Default = configName,
        ...
        Section:AddButton({
            ...
            Callback = function()
                if configName == "" then
                    Nt("Config name cannot be empty!", 3, "Red", "Config", "Error")
                    return
                end
                SaveConfigAs(configName)
                ConfigDropdown:Refresh(GetConfigList())
                Nt("Config saved: " .. configName, 3, "Default", "Config", "Saved")
            end,
            SubTitle = "Load Config",
            SubCallback = function()
                if selectedConfig == "" then
                    Nt("Select a config first!", 3, "Red", "Config", "Error")
                    return
                end
                LoadConfigByName(selectedConfig)
                Nt("Config loaded: " .. selectedConfig, 3, "Default", "Config", "Loaded")
            end
        })
        ...
        Section:AddButton({
            ...
            Callback = function()
                ConfigDropdown:Refresh(GetConfigList())
                Nt("Config list refreshed!", 2, "Default", "Config", "Refreshed")
            end,
            ...
            SubCallback = function()
                ...
                local path = ConfigFolder .. "/Config/" .. selectedConfig .. ".json"
                if isfile and isfile(path) then
                    delfile(path)
                    ConfigDropdown:Refresh(GetConfigList())
                    selectedConfig = ""
                    Nt("Config deleted: " .. selectedConfig, 3, "Default", "Config", "Deleted")
                end
            end
        })
    end
    ...
    local function getConfigList()
        ...
        for _, file in ipairs(listfiles(folder)) do
            ...
            if name and name ~= ("CHX_" .. (ConfigFile:match("CHX_(.+)%.json$") or "")) then
                table.insert(list, name)
            end
        end
        return list
    ...
    local function saveConfigAs(name)
        ...
        if not isfolder(folder) then makefolder(folder) end
        ConfigData._version = CURRENT_VERSION
        writefile(folder .. "/" .. name .. ".json", HttpService:JSONEncode(ConfigData))
        return true
    end
    ...
    local function loadConfigAs(name)
        ...
        if ok and type(result) == "table" then
            for key, element in pairs(Elements) do
                if result[key] ~= nil and element.Set then
                    element:Set(result[key], true)
                end
            end
            return true
        end
    ...
    local function deleteConfigAs(name)
        if not name or name == "" then return false end
        local path = ConfigFolder .. "/Config/" .. name .. ".json"
        if isfile(path) then
            delfile(path)
    ...
    -- ✅ SetupConfig: buat tab config otomatis
    function GuiFunc:SetupConfig(SectionAdd, DropdownRef)
        local currentConfigName = LocalPlayer.Name
        local configDropdown = DropdownRef
        ...
        return {
            Save = function()
                if saveConfigAs(currentConfigName) then
                    Nt("Config '" .. currentConfigName .. "' saved!", 3, "Default", "Config", "Saved")
                    refreshDropdown()
                end
            end,
            Load = function(name)
                if loadConfigAs(name or currentConfigName) then
                    Nt("Config '" .. (name or currentConfigName) .. "' loaded!", 3, "Default", "Config", "Loaded")
                end
            end,
            Delete = function(name)
                if deleteConfigAs(name or currentConfigName) then
                    Nt("Config '" .. (name or currentConfigName) .. "' deleted!", 3, "Default", "Config", "Deleted")
                    refreshDropdown()
                end
            end,
    ...
    -- ✅ MakeConfigSection: buat section config siap pakai
    function GuiFunc:MakeConfigSection(Tab)
        local Sec = Tab:AddSection({ Title = "Config", Icon = "solar:settings-bold" })

        local configName = LocalPlayer.Name
        local configList = getConfigList()
        local selectedConfig = ""
        local configHandler = nil
        ...
        Sec:AddInput({
            ...
            Default = configName,
            Callback = function(value)
                configName = value ~= "" and value or LocalPlayer.Name
                if configHandler then
                    configHandler.SetName(configName)
                end
            end
        })

        -- Dropdown pilih config
        local dropRef = {}
        local DropFrame -- referensi ke dropdown element

        DropFrame = Sec:AddDropdown({
            Title = "Select Config",
            Content = "Choose a saved config",
            Options = configList,
            Multi = false,
        ...
        dropRef.Refresh = function(newList)
            if DropFrame and DropFrame.Refresh then
                DropFrame:Refresh(newList)
            end
        end

        configHandler = {
            Save = function()
                if saveConfigAs(configName) then
                    Nt("Config '" .. configName .. "' saved!", 3, "Default", "Config", "Saved")
            ...
            Load = function()
                local target = selectedConfig ~= "" and selectedConfig or configName
                if loadConfigAs(target) then
                    Nt("Config '" .. target .. "' loaded!", 3, "Default", "Config", "Loaded")
                end
            end,
            Delete = function()
                local target = selectedConfig ~= "" and selectedConfig or configName
                if deleteConfigAs(target) then
                    Nt("Config '" .. target .. "' deleted!", 3, "Default", "Config", "Deleted")
                    local newList = getConfigList()
                    if DropFrame and DropFrame.Refresh then
                        DropFrame:Refresh(newList)
                    end
            ...
            SetName = function(name) configName = name end,
            GetName = function() return configName end,
        }
        ...
        Sec:AddButton({
            ...
            Callback = function()
                Chloex:Dialog({
                    Title = "Delete Config",
                    Content = "Are you sure you want to delete '" .. (selectedConfig ~= "" and selectedConfig or configName) .. "'?",
                    Buttons = {
                        {
                            Name = "Delete",
                            Callback = function()
    ...
    -- ✅ Merge GuiFunc methods ke Tabs agar Window:SetupConfig(), Window:SetToggleKey() dll bisa dipanggil
