...
function Chloex:Window(GuiConfig)
    ...
    function Tabs:AddTab(TabConfig)
        ...
        function Sections:AddSection(SectionConfig)
            ...
            -- ✅ FIX: Pakai AbsoluteContentSize dari UIListLayout (dihitung otomatis Roblox)
            if OpenSection then
                task.spawn(function()
                    -- Tunggu semua child selesai di-add
                    local lastChildTime = tick()
                    local conn = SectionAdd.ChildAdded:Connect(function()
                        lastChildTime = tick()
                    end)
                    repeat task.wait(0.05) until tick() - lastChildTime >= 0.15
                    conn:Disconnect()

                    -- Tunggu Roblox selesai hitung AbsoluteContentSize
                    -- dengan cara nunggu sampai nilainya stabil
                    local lastSize = -1
                    repeat
                        task.wait(0.05)
                        local currentSize = UIListLayout2.AbsoluteContentSize.Y
                        if currentSize == lastSize then break end
                        lastSize = currentSize
                    until false

                    -- Pakai AbsoluteContentSize yang sudah dihitung Roblox
                    local contentHeight = UIListLayout2.AbsoluteContentSize.Y
                    local totalHeight = 38 + contentHeight

                    FeatureFrame.Rotation = 90
                    Section.Size = UDim2.new(1, 1, 0, totalHeight)
